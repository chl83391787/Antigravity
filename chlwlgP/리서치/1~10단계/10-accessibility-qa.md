...
## 4. 600선 확장 접근성 & 성능 표준 (신규 추가)
5. **모바일 터치 타겟 규격**: 모든 인터랙션 버튼 최소 `44px x 44px` 터치 면적 확보.
6. **Lighthouse 최적화**: Performance 90+, FCP 1.2s 이하, CLS 0.05 미만 유지.
7. **모달 포커스 트랩 (Focus Trap)**: [Radix UI](https://www.radix-ui.com/) 표준 준수로 모달 열림 시 배경 포커스 완벽 차단.
