...
* **구성 요점**:
  * GNB 6대 메뉴 헤더, 물 속 원단 모션 비주얼 플레이스홀더 히어로 ([Izanami](https://izanami-official.com/))
...
* **인터랙션 명세**:
  * 좌측: 돋보기 호버 시 500% HD Magnifier 생성 ([Bottega](https://www.bottegaveneta.com/)), 세탁 전/후 1:1 수축 비교 Slider
  * 우측: 실시간 Canvas 자수 입력창(문구/폰트/컬러), 낱장/세트 장당 단가 계산기, N Pay 1초 결제 버튼
  * 유효성 검사: 자수 문구 15자 초과 시 `"자수 문구는 최대 15자까지 입력 가능합니다."` 메세지 출력

---

## 3. 모바일 및 마이크로 인터랙션 명세 (신규 추가)
* **P-05 필터 바텀시트**: 모바일 뷰포트에서 [Vaul](https://vaul.emilkowal.ski) 스타일의 한 손 조작 바텀시트 드로어 지원.
* **P-07 모바일 스티키 CTA**: 화면 하단에 `장바구니 담기` 및 `N Pay 1초 결제` 고정 바 제공.
* **알림 피드백**: [Sonner](https://sonner.emilkowal.ski) 스타일 스택형 토스트 알림 연동.
